...
function MovieProvider(props) {
    ...
    useEffect(() => {
        const currentColection = collectionList[0].value;
        setCollection(currentColection);
        const { data } = useSWR(`https://api.themoviedb.org/3/movie/${collection}?api_key=${apiKey}`, fetcher)
