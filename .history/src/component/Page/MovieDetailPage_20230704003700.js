    ...
    if (video)
        return (
            <div className='p-3 w-full aspect-video rounded-lg bg-black bg-opacity-30 mb-8'>
                ...
                <iframe className='rounded-sm w-full h-full object-fill' src={`https://www.youtube.com/embed/${videoKey}`} allowFullScreen></iframe>
            </div>
        )
}
                    ...
                    >
                        {movies && movies.length > 0 ? movies.map((item) => (
                            <SwiperSlide key={item.id}>
                                <div className={`item-card ${backdrop && data.id === backdrop.id ? "active" : null}`} onClick={() => setBackdrop(data)}>
