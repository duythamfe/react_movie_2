...
function MovieCredit() {
    const { movie_id } = useParams();
    const { data } = useSWR(tmdbAPI.movieCredit(movie_id), fetcher);
...
function VideoPlay() {
    const { movie_id } = useParams();
    const { data } = useSWR(tmdbAPI.videoPlay(movie_id), fetcher);
...
function MovieSimilar() {
    const { movie_id } = useParams();
    const { data } = useSWR(tmdbAPI.moviesSimilar(movie_id), fetcher);
